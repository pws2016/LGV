...
class OrdreCity extends BaseController
{
			...
			switch($this->request->getVar('action')){
				case 'add':
					...
					$tab=array('title'=>$this->request->getVar('title')
						...
						$x=$this->OrdreCityModel->insert($tab);
				...
				case 'edit':
					...
					$tab=array('title'=>$this->request->getVar('title'),
						...
						$this->OrdreCityModel->update($this->request->getVar('id'),$tab);
				...
				case 'delete':
					...
					if($user_to_delete!=""){
						$this->OrdreCityModel->where('id',$user_to_delete)->delete();
						$data['success']=lang('app.success_delete');
					}
				break;
			}
		}
		$ll=$this->OrdreCityModel;
		if($this->request->getVar('search')!==null){
			$st=$this->request->getVar('search_text');
			if($st!=""){
				$data['search_text']=$st;
				$ll=$this->OrdreCityModel->like('title',$st);
			}
			
		}
		$ll=$this->OrdreCityModel->find();
		
		$data['list']=$ll;
		
		return view('admin/ordre_city',$data);
	}
	
	public function update(){
		$id=$this->request->getVar('id');
		$inf=$this->OrdreCityModel->find($id);
